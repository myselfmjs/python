...
from python.wymusic import html_downloader
...
def AESencrypt(msg, key):
    ...
    msg = msg + padding * chr(padding).encode('utf-8')
...
if __name__ == '__main__':
    ...
    hotComments = json.loads(r.text)['hotComments']

    for i in range(0,len(hotComments)):
        userName = hotComments[i]['user']['nickname']
        userId = hotComments[i]['user']['userId']
        likedCount = hotComments[i]['likedCount']
        content = hotComments[i]['content']
        print('评论人：' + userName)
        print('评论人ID：' + str(userId))
        print('评论点赞数：' + str(likedCount))
        print('评论内容：' + content)
        print(' ')
